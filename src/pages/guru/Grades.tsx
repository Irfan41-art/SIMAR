...
export default function Grades() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-display font-black text-white tracking-tight uppercase">Penilaian Murid</h1>
        <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mt-1">Evaluasi akademik dan rekapitulasi performa belajar</p>
      </div>

      <div className="flex gap-2 p-1.5 bg-slate-900/40 backdrop-blur-md rounded-2xl w-fit border border-white/5 shadow-2xl">
        <button 
          onClick={() => setActiveTab('input')}
          className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'input' ? 'bg-blue-600 text-white shadow-xl shadow-blue-900/20' : 'text-slate-500 hover:text-slate-300'}`}
        >
          Input Nilai
        </button>
        <button 
          onClick={() => setActiveTab('report')}
          className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'report' ? 'bg-blue-600 text-white shadow-xl shadow-blue-900/20' : 'text-slate-500 hover:text-slate-300'}`}
        >
          Rekapitulasi Nilai
        </button>
      </div>
      ...
      {activeTab === 'input' ? (
        <div className="space-y-6">
          <div className="bg-slate-900/40 backdrop-blur-xl p-8 rounded-[2rem] border border-white/5 border-l-4 border-l-blue-600 shadow-2xl shadow-black/20">
            ...
            <div className="mt-8">
              <button 
                ...
                className="bg-blue-600 text-white px-8 py-4 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2 hover:bg-blue-500 hover:shadow-xl hover:shadow-blue-500/20 transition-all active:scale-95 shadow-lg shadow-black/20"
              >
              ...
              >
                ...
                <div className="p-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {students.map(s => (
                      <div key={s.id} className="p-5 bg-slate-950/50 rounded-2xl flex items-center justify-between border border-white/5 group hover:border-blue-500/30 transition-all">
                        <span className="text-[11px] font-black text-slate-400 group-hover:text-slate-200 truncate mr-3 uppercase tracking-wide">{s.name}</span>
                        <input 
                          type="number"
                          min="0" max="100"
                          value={records[s.id] || ''}
                          onChange={(e) => setRecords({...records, [s.id]: parseInt(e.target.value) || 0})}
                          className="w-20 px-3 py-2 bg-slate-900 border border-white/5 rounded-xl text-center font-black text-blue-400 focus:ring-2 focus:ring-blue-500 outline-none text-sm shadow-inner"
                        />
                      </div>
                    ))}
                  </div>
                ...
                <div className="p-8 bg-white/5 border-t border-white/5 flex justify-end">
                  <button 
                    onClick={handleSave}
                    disabled={saving}
                    className="bg-blue-600 text-white px-12 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-3 hover:bg-blue-500 hover:shadow-xl hover:shadow-blue-500/20 active:scale-95 transition-all shadow-lg shadow-black/40 disabled:opacity-50"
                  >
        ...
        <div className="bg-slate-900/40 backdrop-blur-xl p-12 rounded-[3rem] border border-white/5 shadow-2xl shadow-black/40 max-w-lg mx-auto border-t-4 border-t-blue-600">
          <div className="text-center mb-10">
            <div className="w-16 h-16 bg-blue-600/10 rounded-2xl flex items-center justify-center text-blue-400 mx-auto mb-6 border border-blue-400/10 shadow-[0_0_20px_rgba(59,130,246,0.1)]">
              <Printer size={32} />
            </div>
            <h3 className="text-xl font-display font-black text-white uppercase tracking-tight">Dokumen Rekapitulasi</h3>
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mt-2">Dapatkan laporan nilai kolektif per kategori</p>
          </div>
