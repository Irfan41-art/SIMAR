...
export default function Login({ role: initialRole }: { role?: 'admin' | 'guru' }) {
  ...
  const { settings, user, profile, loading: appLoading } = useAppContext();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEmailInput) {
        // Jika input adalah email, kita cari di Firestore berdasarkan email atau username
        const snap = await getDocs(query(collection(db, 'users'), where('email', '==', inputVal)));
        if (!snap.empty) {
          userData = snap.docs[0].data();
          authEmail = inputVal;
        } else {
          // Coba login langsung jika tidak ada di firestore (mungkin user baru)
          authEmail = inputVal;
        }
      } else {
        // Input adalah Username atau NIP
        const inputLower = inputVal.toLowerCase();
        let snap = await getDocs(query(collection(db, 'users'), where('username', '==', inputVal)));
        
        if (snap.empty) {
          snap = await getDocs(query(collection(db, 'users'), where('username', '==', inputLower)));
        }
        
        if (snap.empty) {
          snap = await getDocs(query(collection(db, 'users'), where('nip', '==', inputVal)));
        }
        
        if (!snap.empty) {
          const userDoc = snap.docs[0];
          userData = userDoc.data();
          // Gunakan email terdaftar jika ada, jika tidak gunakan format .local
          authEmail = userData.email || `${userData.username.toLowerCase().replace(/\s+/g, '')}@simar.local`;
        }
      }

      // Kasus Khusus: Admin Default (fallback jika db kosong)
      if (inputVal === 'admin' && cleanPassword === 'admin123' && isAdminPage) {
        authEmail = 'admin@simar.local';
      }

      if (!userData && !isEmailInput && inputVal !== 'admin') {
        setError('Akun tidak ditemukan. Silakan hubungi Admin.');
        setLoading(false);
        return;
      }

      // VALIDASI: Pastikan role cocok dengan halaman login (kecuali jika login dengan email langsung yang mungkin belum ada di Firestore)
      if (userData && userData.role !== role) {
        setError(`Akun ini terdaftar sebagai ${userData.role === 'admin' ? 'Admin' : 'Guru'}. Silakan gunakan portal login yang sesuai.`);
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        // Sinkronisasi data Firestore jika login berhasil tetapi ID dokumen belum sinkron
        if (userData) {
          const userSnap = await getDocs(query(collection(db, 'users'), where('username', '==', userData.username)));
          if (!userSnap.empty) {
            const currentDoc = userSnap.docs[0];
            if (currentDoc.id !== loginUid) {
              await setDoc(doc(db, 'users', loginUid), {
                ...currentDoc.data(),
                uid: loginUid,
                lastLogin: new Date().toISOString()
              }, { merge: true });
              await updateDoc(doc(db, 'users', currentDoc.id), { migrated: true, migratedTo: loginUid });
            } else {
              await updateDoc(doc(db, 'users', currentDoc.id), { lastLogin: new Date().toISOString() });
            }
          }
        }
      } catch (authErr: any) {
        // Jika user belum ada di Auth tapi ada di Firestore (Migrasi Otomatis)
        if (userData && userData.password === cleanPassword && 
           (authErr.code === 'auth/user-not-found' || authErr.code === 'auth/invalid-credential' || authErr.code === 'auth/invalid-email')) {
          
          try {
            const userCredential = await createUserWithEmailAndPassword(auth, authEmail, cleanPassword);
            const newUid = userCredential.user.uid;

            // Update document user dengan UID baru agar sinkron dengan Auth
            const userSnap = await getDocs(query(collection(db, 'users'), where('username', '==', userData.username)));
            if (!userSnap.empty) {
              const docRef = doc(db, 'users', userSnap.docs[0].id);
              // Jika ID dokumen bukan UID, kita migrasikan ke dokumen baru dengan ID = UID
              if (userSnap.docs[0].id !== newUid) {
                await setDoc(doc(db, 'users', newUid), {
                  ...userData,
                  uid: newUid,
                  lastLogin: new Date().toISOString()
                });
                // Kita tidak menghapus dokumen lama karena mungkin ada data relasional, 
                // tapi kita tandai sebagai migrated.
                await updateDoc(docRef, { migrated: true, migratedTo: newUid });
              } else {
            ...
            setError(`Gagal sinkronisasi keamanan: ${createErr.message}`);
          }
        } else {
          setError('Kredensial tidak valid. Periksa kembali Username/NIP dan Password Anda.');
        }
      }
    } catch (err: any) {
      setError(`Kesalahan sistem: ${err.message || 'Coba lagi nanti'}`);
    } finally {
      setLoading(false);
    }
  };
